...
class ResidualsPreprocessing(PreprocessingBase):
    ...
    def fit(self, train, test=None):
        train_factors_merged = train.merge(self._factors, right_index=True, left_index=True,
                                           suffixes=['', '_factor'])
        factors_data = train_factors_merged[self._factors.columns + '_factor']
        train_data = train_factors_merged.drop(columns=self._factors.columns + '_factor')
    ...
    def transform(self, test):
        ...
        test_factors_merged = test.merge(self._factors, right_index=True, left_index=True,
                                         suffixes=['', '_factor'])
        factors_data = test_factors_merged[self._factors.columns + '_factor']
        test_data = test_factors_merged.drop(columns=self._factors.columns + '_factor')
